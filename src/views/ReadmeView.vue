...
<template>
    <div v-show="store.state.view.type === 'readme'">
        ...
        <p>除此之外，你可能需要注意间接数据的不确定度计算方式:</p>
        <ul>
            <li>
                当选择<em>由计算式推导</em>时，处理器会自动根据计算式求出间接数据的不确定度。这时，你应当确保每一个参与运算的参数的不确定度是有意义的，否则计算出来的不确定度并不可信。
            </li>
            <li>
                当选择<em>重新计算</em>时，会重新计算数据的A类不确定度，且允许再填入仪器允差。这在处理如 <vue-latex expression="(x_1 - x_2)"></vue-latex> 的数据时较为合适。
            </li>
        </ul>
