...
const commentFormat = (str) => {
    ...
    function protectTitle(input) {
        // 按照给定的符号和函数名称进行部分替换
        let result = titleFormat(input).replace(/\+/g, '@ADD')
    ...
    function restoreTitle(input) {
        ...
        let result = input.replace(/@ABS/g, 'abs')
            .replace(/@LN/g, 'ln')
            .replace(/@LG/g, 'lg')
            .replace(/@SQRT/g, 'sqrt')
            .replace(/@ADD/g, '+')
...
<div v-show="isReadme">
    ...
    <h2>设置</h2>
    <p>在边栏的设置中可以选择文件保存策略和输出语言。输出语言决定了制表和制图时一些默认注释的语言。这些设置将被保存在 package.nw（mac 用户为 app.nw）同级目录 user 下的 config 目录中。</p>
